...
public:
	...
	DNA& readFASTA (const char* filename) {
		...
		while (!in->eof() ) {
			...
			if ( (char) str == '>') {
				...
				if (coutput) {
					cout << "Sequences are " << lseq << " long" << endl;
				}
				break;
			}
		}
	...
	DNA& resize (const int NSEQ, const int LSEQ) {
		if (NSEQ < 0) {
			error ("DNA: NSEQ must be non-negative");
		}
		if (LSEQ < 0) {
			error ("DNA: LSEQ must be non-negative");
		}
		nseq = NSEQ;
		lseq = LSEQ;
	...
	double H() {
		...
		for (i = 1; i < nseq; i++) {
			...
			for (ii = 0; ii < result; ii++) {
				for (j = 0; j < lseq; j++)
					if (sequence[i][j] != sequence[uniqueHaps[ii]][j]) {
						break;
					}
				if (j == lseq) {
					unique = false;
	...
	double Rm() {
		...
		for (j = 0; j < S; j++)
			for (k = 0; k < j; k++) {
				...
				for (i = 1; i < nseq; i++) {
					if (sequence[i][_sites[j]] == hap0 && sequence[i][_sites[k]] != hap1) {
						comb[0] = true;
					}
					if (sequence[i][_sites[j]] != hap0 && sequence[i][_sites[k]] == hap1) {
						comb[1] = true;
		...
		for (i = S - 3; i >= 0; i--) {
			...
			for (k = i + 2; k < S; k++) if (_B_B[k][i] + _M[k] > _M[i]) {
					_M[i] = _B_B[k][i] + _M[k];
				}
		}
	...
	void RecCorrelations (double* result, bool normalize) {
		...
		for (j = 0; j < S; j++)
			for (k = 0; k < j; k++) {
				...
				for (i = 0; i < nseq; i++) {
					if (sequence[i][_sites[j]] == hap0 && sequence[i][_sites[k]] == hap1) {
						++_G[j][k][0];
					} else if (sequence[i][_sites[j]] == hap0 && sequence[i][_sites[k]] != hap1) {
						++_G[j][k][1];
					} else if (sequence[i][_sites[j]] != hap0 && sequence[i][_sites[k]] == hap1) {
						++_G[j][k][2];
					} else if (sequence[i][_sites[j]] != hap0 && sequence[i][_sites[k]] != hap1) {
		...
		for (i = 0, ctr = 0; i < S; i++)
			for (j = 0; j < i; j++, ctr++) {
				...
				E[3] += _D[i][j];
				EE[0] += _A[i][j] * _A[i][j];
				EE[1] += B_B[i][j] * B_B[i][j];
				EE[2] += _CC[i][j] * _CC[i][j];
				EE[3] += _D[i][j] * _D[i][j];
				ED[0] += _A[i][j] * _D[i][j];
				ED[1] += B_B[i][j] * _D[i][j];
				ED[2] += _CC[i][j] * _D[i][j];
