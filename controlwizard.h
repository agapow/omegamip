...
public:
	...
	void read_input (const char* filename) {
		...
		if (coutput) {
			printf ("Finished reading in control file.\n\n");
		}
	...
	//Returns false if some required items are not found
	{
		if (!got_required) {
			...
			printf ("All required items were found\n");
		}
		return got_required;
	}
...
protected:
	...
	/*Returns true if a label is found*/
	{
		...
		if (!case_sensitive) {
			...
			int ii;
			for (ii = 0; ii < (int) word.length(); ii++) {
...
//	RTRV (ControlWizard::*read_data)(std::ifstream &infile, std::string &label);
	void read_data (std::ifstream &infile, std::string &label) {
		switch (switcher) {
		case TP_UNRECOGNISED:
			function_get_unrecognised (infile, label);
			break;
		case TP_INT:
			function_get_int (infile, label);
			break;
		case TP_DOUBLE:
			function_get_double (infile, label);
			break;
		case TP_STRING:
			function_get_string (infile, label);
			break;
		case TP_VEC_INT:
			function_get_vector_int (infile, label);
			break;
		case TP_VEC_DOUBLE:
			function_get_vector_double (infile, label);
			break;
		case TP_EXT_VEC_DOUBLE:
			function_get_external_vector_double (infile, label);
			break;
		case TP_VEC_STRING:
			function_get_vector_string (infile, label);
			break;
		default:
			function_get_unrecognised (infile, label);
			break;
	...
	RTRV function_get_external_vector_double (std::ifstream &infile, std::string &label) {
		...
		if (extfile.is_open() == false) {
			error ("function_get_external_vector_double():External file not found");
		}
