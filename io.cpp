...
omegaMapBase& omegaMapBase::record (ofstream& out, oEvent& event, ostream& omanip (ostream&) ) {
	...
	for (i = 0; i < field.size(); i++) {
		...
		case -1:
			...
			break;
		case 0:
			out << _block[0].oMat->mu;
			break;
		case 1:
			out << _block[0].oMat->kappa;
			break;
		case 2:
			out << indelLambda;
			break;
		...
		case 10:
			...
			break;
		case 11:
			out << (block[k]->start == k) ? 1 : 0;
			break;
		case 12:
			out << block[k]->oMat->omega;
			break;
		case 13:
			out << (rblock[k]->start == k) ? 1 : 0;
			break;
		case 14:
			out << rblock[k]->rho;
			break;
	...
	out << endl;
	if (out.fail() ) {
		error ("omegaMapBase::record(): write fail");
	}
	return *this;
}
...
ostream& operator<< (ostream& out, omegaMapBase& oM) {
	...
	if (!oM.writeBinary) {
		...
		for (i = 0; i < oM.niter; i++) {
			out << oM.event[i];
			if (out.fail() ) {
				error ("ostream& operator<<(ostream&, omegaMapBase&): write failure");
			}
		}
	} else {
#ifdef __GNUC__
...
istream& operator>> (istream& in, omegaMapBase& oM) {
	...
	if (oM.ran != NULL) {
		oM.ran->setseed (temp_seed);
	}
	for (i = 0; i < 5; i++) {
		if (varPrior[i] == 0) {
			*refPrior[i] = omegaMapBase::none;
		} else if (varPrior[i] == 1) {
			*refPrior[i] = omegaMapBase::fixed;
		} else if (varPrior[i] == 2) {
			*refPrior[i] = omegaMapBase::uniform;
		} else if (varPrior[i] == 3) {
			*refPrior[i] = omegaMapBase::improper_uniform;
		} else if (varPrior[i] == 4) {
			*refPrior[i] = omegaMapBase::exponential;
		} else if (varPrior[i] == 5) {
			*refPrior[i] = omegaMapBase::gamma;
		} else if (varPrior[i] == 6) {
			*refPrior[i] = omegaMapBase::exponential_ratio;
		} else if (varPrior[i] == 7) {
			*refPrior[i] = omegaMapBase::inverse;
		} else if (varPrior[i] == 8) {
			*refPrior[i] = omegaMapBase::improper_inverse;
		} else if (varPrior[i] == 9) {
			*refPrior[i] = omegaMapBase::log_normal;
		} else {
			error ("istream& operator>>(istream&, omegaMapBase&): unrecognised prior");
		}

		if (varPriorParam[i].size() != 2) {
			error ("istream& operator>>(istream&, omegaMapBase&): PriorParam not found");
		}
...
//	out.write((char*)&event,sizeof(oEventStartBinary));
	...
	int i;
	for (i = 0; i < event.nblocks; i++) {
		out.write ( (char*) &event.oBlockStart[i], sizeof (event.oBlockStart[i]) );
	}
	for (i = 0; i < event.nblocks; i++) {
		out.write ( (char*) &event.omega[i], sizeof (event.omega[i]) );
	}
	for (i = 0; i < event.nrblocks; i++) {
		out.write ( (char*) &event.rBlockStart[i], sizeof (event.rBlockStart[i]) );
	}
	for (i = 0; i < event.nrblocks; i++) {
		out.write ( (char*) &event.rho[i], sizeof (event.rho[i]) );
	}
	return out;
}
...
//	in.read((char*)&event,sizeof(oEventStartBinary));
	...
	int i;
	for (i = 0; i < event.nblocks; i++) {
		in.read ( (char*) &event.oBlockStart[i], sizeof (event.oBlockStart[i]) );
	}
	for (i = 0; i < event.nblocks; i++) {
		in.read ( (char*) &event.omega[i], sizeof (event.omega[i]) );
	}
	for (i = 0; i < event.nrblocks; i++) {
		in.read ( (char*) &event.rBlockStart[i], sizeof (event.rBlockStart[i]) );
	}
	for (i = 0; i < event.nrblocks; i++) {
		in.read ( (char*) &event.rho[i], sizeof (event.rho[i]) );
	}
	return in;
}
...
ostream& operator<< (ostream& out, oEventStart& event) {
	...
	int i;
	for (i = 0; i < event.nblocks; i++) {
		out << event.oBlockStart[i] << tab;
	}
	for (i = 0; i < event.nblocks; i++) {
		out << event.omega[i] << tab;
	}
	for (i = 0; i < event.nrblocks; i++) {
		out << event.rBlockStart[i] << tab;
	}
	for (i = 0; i < event.nrblocks; i++) {
		out << event.rho[i] << tab;
	}
...
istream& operator>> (istream& in, oEvent& event) {
	...
	case 0:
		...
		break;
	case 1:
		event.type = oEvent::START;
		break;
	case 2:
		event.type = oEvent::CHANGE_OBLOCK;
		break;
	case 3:
		event.type = oEvent::EXTEND_OBLOCK;
		break;
	case 4:
		event.type = oEvent::SPLIT_OBLOCK;
		break;
	case 5:
		event.type = oEvent::MERGE_OBLOCK;
		break;
	case 6:
		event.type = oEvent::CHANGE_MU;
		break;
	case 7:
		event.type = oEvent::CHANGE_KAPPA;
		break;
	case 8:
		event.type = oEvent::CHANGE_RBLOCK;
		break;
	case 9:
		event.type = oEvent::EXTEND_RBLOCK;
		break;
	case 10:
		event.type = oEvent::SPLIT_RBLOCK;
		break;
	case 11:
		event.type = oEvent::MERGE_RBLOCK;
		break;
	case 12:
		event.type = oEvent::CHANGE_INDEL;
		break;
	case 13:
		event.type = oEvent::CHANGE_ORDER;
		break;
	default:
		event.type = oEvent::DEFAULT;
...
istream& operator>> (istream& in, oEventStart& event) {
	...
	if (in.fail() ) {
		error ("istream& operator>>(istream&, oEventStart&): read failure");
	}
	if (event.nblocks <= 0) {
		error ("istream& operator>>(istream&, oEventStart&): nblocks must be positive");
	}
	if (event.nrblocks <= 0) {
		error ("istream& operator>>(istream&, oEventStart&): nrblocks must be positive");
	}
	...
	int i;
	for (i = 0; i < event.nblocks; i++) {
		in >> event.oBlockStart[i];
	}
	for (i = 0; i < event.nblocks; i++) {
		in >> event.omega[i];
	}
	for (i = 0; i < event.nrblocks; i++) {
		in >> event.rBlockStart[i];
	}
	for (i = 0; i < event.nrblocks; i++) {
		in >> event.rho[i];
	}
