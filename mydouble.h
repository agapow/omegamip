...
public:
	...
	mydouble (const double &_doub) {
		zero = false;
		if (_doub < 0.0) {
			myutils::error ("mydouble::mydouble(const double&): cannot initialize with negative number");
		}
		if (_doub == 0.0) {
			setzero();
		} else {
	...
	mydouble& operator= (const double &_doub) {
		zero = false;
		if (_doub < 0.0) {
			myutils::error ("mydouble::operator=(const double&): cannot assign a negative number");
		}
		if (_doub == 0.0) {
			setzero();
		} else {
	...
	mydouble operator/ (const mydouble &mydbl) const {
		...
		if (zero) {
			...
			error ("mydouble::operator/(const mydouble&): division by zero");
		} else {
			a.setlog (_log - mydbl._log);
		}
		return a;
	...
	mydouble& operator/= (const double &dbl) {
		if (dbl == 0.0) {
			error ("mydouble::operator/=(const double&): division by zero");
		} else if (!zero) {
			_log -= mydouble (dbl)._log;
		}
		return *this;
	...
	mydouble operator+ (const mydouble &mydbl) const {
		...
		if (zero) {
			...
			if (diff == 0.0) {
				a.setlog (log (2.0) + _log);
			} else if (diff < 0.0) {
				a.setlog (mydbl._log + log (1.0 + exp (diff) ) );
			} else {
				a.setlog (_log + log (1.0 + exp (-diff) ) );
			}
		}
		return a;
	}
		...
		if (zero) {
			*this = mydbl;
			...
			if (diff == 0.0) {
				...
				_log = mydbl._log + log (1.0 + exp (diff) );
			} else {
				_log += log (1.0 + exp (-diff) );
			}
		}
		return *this;
	}
	...
	mydouble operator- (const mydouble &mydbl) const {
		...
		if (mydbl.zero) {
			...
			error ("mydouble::operator-(const mydouble&): subtracting a positive number from zero");
		} else {
			...
			if (diff == 0.0) {
				...
				myutils::error ("mydouble::operator-(const mydouble&) cannot handle negative numbers");
			} else {
				a.setlog (_log + log (1.0 - exp (-diff) ) );
			}
		}
