...
double omegaMap::mutProb (int site, int nhaps, int i, int j) {
#ifdef _DEBUG
	if (j > i) {
		warning ("omegaMap::mutProb(): j must be less than or equal to i");
	}
	if (j < 0) {
		warning ("omegaMap::mutProb(): j must be greater than or equal to 0");
	}
	if (i >= maxCodon) {
	...
	if (!indel[site]) {
		...
		if (pr == -1.0) {
			...
			int k;
			for (k = 0; k < 61; k++) {
				pr += oM.R[i][k] * oM.R[j][k] * sqrtPi[j] / sqrtPi[i] * (double) nhaps / ( (double) nhaps - 2.0 * oM.lambda[k]);
			}
			pr *= (i == j) ? pi[i] : 2.0 * pi[i];
			if (pr <= 0.0) {
				pr = numeric_limits<double>::epsilon();
			}
		}
		return pr;
	} else {
				...
				if (pos == 0) {
					for (x = 0; x < k; x++) {
						...
						if (x == 0) {
							...
							alpha[ord][k][pos][k] += log (1.0 + exp (diff) );
						} else {
							alpha[ord][k][pos][k] = alpha[ord][k][pos][x] + log (1.0 + exp (-diff) );
						}
					}
				} else {
					for (x = 0; x < k; x++) {
			...
			for (pos = _3primeMax; pos >= _5primeMin; pos--) {
				...
				if (pos == L - 1) {
					for (x = 0; x < k; x++) {
						...
						if (x == 0) {
							beta[ord][k][pos][k] = beta[ord][k][pos][x];
						} else if (diff < 0.0) {
							beta[ord][k][pos][k] += log (1.0 + exp (diff) );
						} else {
							beta[ord][k][pos][k] = beta[ord][k][pos][x] + log (1.0 + exp (-diff) );
						}
					}
				} else {
					for (x = 0; x < k; x++) {
			...
			for (anc = 0; anc < 61; anc++) {
				...
				for (hap = 0; hap < n; hap++) {
					...
					if (pr == -1.0) {
						pr = 0.0;
						for (k = 0; k < 61; k++) {
							pr += oM.R[i][k] * oM.R[j][k] * sqrtPi[j] / sqrtPi[i] * exp (oM.lambda[k] * 2.0);
						}
						if (pr <= 0.0) {
							pr = numeric_limits<double>::epsilon();
						}
