...
public:
	...
	void initialize (Vector<DNA*> &temp) {
		nloc = temp.size();
		if (nloc < 1) {
			myutils::error ("MLST::initialize(): must be at least one locus");
		}
		int l;
		n = temp[0]->nseq;
		for (l = 1; l < nloc; l++) if (temp[l]->nseq != n) {
				myutils::error ("MLST(): all loci should have the same number of sequences");
			}
		nhap.resize (nloc);
		allele.resize (nloc);
		...
		for (l = 0; l < nloc; l++) {
			...
			if (check_total != n) {
				myutils::error ("MLST(): problem in counting haplotypes");
			}
			...
			if (hap != nhap[l]) {
				myutils::error ("MLST(): hap and nhap disagree");
			}
			for (; hap < n; hap++) {
				count[l][hap] = 0;
			}
