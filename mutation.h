...
public:
	...
	void set_state_freq (vector<double> state_freq_in) {
		if (state_freq_in.size() != n_states) {
			error ("Mutation_Matrix::set_state_freq(state_freq_in): state_freq_in inconsistent in size with n_states");
		}
		double tot = 0.0;
		int i;
		for (i = 0; i < n_states; i++) {
		...
		for (i = 0; i < n_states; i++) {
			state_freq[i] = state_freq_in[i] / tot;
		}
	}
	void set_state_char (vector<string> state_char_in) {
		if (state_char_in.size() != n_states) {
			error ("Mutation_Matrix::set_state_char(state_char_in): state_char_in inconsistent in size with n_states");
		}
	...
	inline double get_rate (const int state) {
		if (state < 0 || state >= n_states) {
			error ("Mutation_Matrix::get_rate(state): unknown state");
		}
	...
	int mutate (const int state) {
		if (state < 0 || state >= n_states) {
			error ("Mutation_Matrix::mutate(state): unknown state");
		}
		int new_state = state;
		double U = ran->U();
		double rp;
	...
	int mutate_edge (const int state, const double time) {
		int old_state = state;
		if (old_state < 0 || old_state >= n_states) {
			error ("Mutation_Matrix::mutate_edge(state,time): unknown state");
		}
		if (time < 0.0) {
			error ("Mutation_Matrix::mutate_edge(state,time): time must be non-negative");
		}
