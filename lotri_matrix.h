...
public:
	...
	LowerTriangularMatrix<T>& initialize (int n) {
		...
		array = new T[size];
		if (!array) {
			error ("array allocation failure in LowerTriangularMatrix::initialize()");
		}

		element = new T*[n];
		if (!element) {
			error ("element allocation failure in LowerTriangularMatrix::initialize()");
		}
		for (i = 0; i < n; i++) {
			element[i] = & (array[i* (i+1) /2+0]);
		}
	...
	LowerTriangularMatrix<T>& resize (int n) {
		...
		array = new T[size];
		if (!array) {
			error ("array allocation failure in LowerTriangularMatrix::resize()");
		}

		element = new T*[n];
		if (!element) {
			error ("element allocation failure in LowerTriangularMatrix::resize()");
		}
		for (i = 0; i < n; i++) {
			element[i] = & (array[i* (i+1) /2+0]);
		}
