...
public:
	...
	inline T& operator[] (int pos) {
		if (pos < lo) {
			error ("safeArray::operator[](int pos): pos<min");
		}
		if (pos >= hi) {
			error ("safeArray::operator[](int pos): pos>=max");
		}
		return element[pos];
	}
};
...
public:
	...
	Matrix<T>& initialize (int nrows, int ncols) {
		...
		if (!array) {
			error ("array allocation failure in Matrix::initialize()");
		}

		element = new T*[nrows];
		if (!element) {
			error ("element allocation failure in Matrix::initialize()");
		}
		for (i = 0; i < nrows; i++) {
	...
	Matrix<T>& resize (int nrows, int ncols) {
		...
		if ( (nrows == protected_nrows) && (ncols == protected_ncols) ) {
			return *this;
		}
		...
		array = new T[nrows*ncols];
		if (!array) {
			error ("array allocation failure in Matrix::resize()");
		}
	...
	*/
	...
	inline safeArray< T > operator[] (int pos) {
		...
		if (pos >= protected_nrows) {
			error ("Matrix::operator[](int row): row>=nrows()");
		}
		//return element[pos];
	...
	Matrix<T> operator* (const Matrix<T>& mat) {
		if (ncols() != mat.nrows() ) {
			error ("Matrix multiplication: matrices are not conformable");
		}
	...
	Matrix<T> invert() {
		if (protected_nrows != protected_ncols) {
			error ("Matrix inversion: must be a symmetric matrix");
		}
		Matrix<T> a = *this;
		...
		for (i = 0; i < n; i++) {
			...
			indxc[i] = icol;
			if (a[icol][icol] == 0.0) {
				error ("Matrix inversion: Singular Matrix");
			}
