...
public:
	...
	void read_input (int argc_in, char* argv_in[]) {
		argc = argc_in;
		...
		if (coutput) {
			std::cout << "Finished reading in control file." << std::endl << std::endl;
		}
	...
	bool check_required() {
		if (!got_required) {
			...
			for (i = required.begin(); i != required.end(); i++) {
				std::cout << *i << " ";
			}
			std::cout << std::endl;
		} else {
			std::cout << "All required items were found" << std::endl;
		}
		return got_required;
	}
...
protected:
	...
	bool read_label (std::string &word) {
		if (argn >= argc) {
			error ("Syntax error in ArgumentWizard::read_label: exceeded number of arguments");
		}
		word = argv[argn];
		if (word[0] != '-') {
			error ("Syntax error in ArgumentWizard::read_label: option must be prefixed with a \'-\'");
		}
...
protected:
	...
	RTRV function_get_single (T dummy, std::string &label) {
		if (argn >= argc) {
			error ("Syntax error in ArgumentWizard::function_get_single(): exceeded number of arguments");
		}
	...
	RTRV function_get_vector (T dummy, std::string &label) {
		if (argn >= argc) {
			error ("Syntax error in ArgumentWizard::function_get_vector(): exceeded number of arguments");
		}
		std::string word;
	...
	void read_data (std::string &label) {
		...
		case TP_UNRECOGNISED:
			function_get_unrecognised (label);
			break;
		case TP_INT:
			function_get_single ( (int) 0, label);
			break;
		case TP_DOUBLE:
			function_get_single ( (double) 0, label);
			break;
		case TP_STRING:
			function_get_string (label);
			break;
		case TP_VEC_INT:
			function_get_vector ( (int) 0, label);
			break;
		case TP_VEC_DOUBLE:
			function_get_vector ( (double) 0, label);
			break;
		case TP_EXT_VEC_DOUBLE:
			function_get_external_vector_double (label);
			break;
		default:
			function_get_unrecognised (label);
			break;
